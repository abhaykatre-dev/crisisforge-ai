...
def _check_constraint(patient: Dict, target_hospital: Dict) -> Dict:
    """Lightweight in-engine wrapper around ethical_constraint_check."""
    from fairness_engine import ethical_constraint_check
    return ethical_constraint_check(patient, target_hospital)


def _log_allocation(db, patient: Dict, transfer: Dict) -> None:
    """Persist one AllocationLog row after a successful transfer decision."""
    from models import AllocationLog
    log = AllocationLog(
        patient_id=patient.get("patient_id", "unknown"),
        source_hospital=transfer["from_hospital"],
        dest_hospital=transfer["to_hospital"],
        timestamp=datetime.utcnow(),
        severity_score=patient.get("severity_score", 5.0),
        patient_origin=patient.get("patient_origin", "urban"),
        wait_time_minutes=transfer.get("estimated_transfer_time_min", 30.0),
        transfer_count=patient.get("transfer_count", 0) + 1,
    )
    try:
        db.add(log)
        db.commit()
...
def recommend_transfers(
    ...
    db=None,          # optional SQLAlchemy Session for logging
) -> Dict:
    """
    ...
    for sender in senders:
        ...
        for scored in receiver_scores[:3]:  # Top 3 receivers per sender
            ...
            # ── FairFlow: ethical constraint check ──
            sample_patient = {
                "patient_id": f"SIM-{transfer_count+1:03d}",
                "transfer_count": 0,
                "needs_icu": transferable_icu > 0,
                "severity_score": 7.5 if transferable_icu > 0 else 5.0,
                "patient_origin": random.choice(["rural", "urban"]),
            }
            target_info = {"name": receiver["name"], "rural_priority": True}
            constraint_result = _check_constraint(sample_patient, target_info)
            if not constraint_result["allowed"]:
                continue  # skip this transfer — ethical rule violated
            ...
            # ── FairFlow: log to AllocationLog if db session provided ──
            if db is not None:
                _log_allocation(db, sample_patient, transfer_record)
