...
from fairness_engine import get_current_fairness_score, audit_bias, ethical_constraint_check
...
# ═══════════════════════════════════════════════════
#  FAIRFLOW FAIRNESS ENGINE ROUTES
# ═══════════════════════════════════════════════════

class ConstraintRequest(BaseModel):
    patient: Dict
    target_hospital: Dict


@app.get("/api/fairness/score")
def fairness_score(db: Session = Depends(get_db)):
    """Return the current overall fairness score (0-1) for the last 24 h of allocations."""
    return get_current_fairness_score(db)


@app.get("/api/fairness/audit")
def fairness_audit(db: Session = Depends(get_db)):
    """Return a detailed bias audit report over the last 100 allocation logs."""
    return audit_bias(db)


@app.post("/api/fairness/constraint")
def fairness_constraint(req: ConstraintRequest):
    """Check whether a proposed transfer violates any ethical constraint."""
    return ethical_constraint_check(req.patient, req.target_hospital)
